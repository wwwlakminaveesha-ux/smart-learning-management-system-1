...
const LecturerResults = () => {
  ...
  const fetchResults = async (selectedModuleId) => {
    if (!selectedModuleId) {
      setResults([]);
      setStudents([]);
      return;
    ...
    try {
      ...
      const res = await api.get(`/results/module/${selectedModuleId}`);
      setResults(res.data);

      const selectedModule = modules.find((m) => m._id === selectedModuleId);
      setStudents(selectedModule?.students || []);
    } catch (err) {
  ...
  const publishResult = async (e) => {
    ...
    if (!moduleId || !studentId) {
      alert("Please select module and student");
      return;
    }
            ...
            <div style={styles.header}>
              <div>
                ...
                <p style={styles.subtitle}>
                  Publish and review module results
                </p>
              </div>
          ...
          <div style={styles.selectorCard}>
            <div style={styles.sectionTop}>
              ...
              <p style={styles.sectionSubtitle}>
                Choose a module to publish and review student results
              </p>
            </div>
          ...
          {moduleId ? (
            <>
              <div style={styles.formCard}>
                <div style={styles.sectionTop}>
                  <h2 style={styles.sectionTitle}>Publish Result</h2>
                  <p style={styles.sectionSubtitle}>
                    Select a student and publish result feedback for the chosen module
                  </p>
                </div>

                <form onSubmit={publishResult} style={styles.form}>
                  <select
                    value={studentId}
                ...
                <div style={styles.tableWrapper}>
                  <div style={styles.tableHeader}>
                    ...
                    <p style={styles.tableSubtitle}>
                      Review result records for students in the selected module
                    </p>
                  </div>
                      ...
                      <tbody>
                        {results.map((result) => (
                          <tr key={result._id} style={styles.tr}>
                            <td style={styles.td}>{result.student?.name || "-"}</td>
                            <td style={styles.td}>
            ...
            !loadingModules && (
              <div style={styles.emptySelectionCard}>
                Please select a module to view and manage results.
              </div>
            )
          )}
        </div>
